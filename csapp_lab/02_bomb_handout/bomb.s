...
phase_2:
  ...
  400f0a:	83 3c 24 01 	cmpl	$1, (%rsp).     #check if the first number==1(so the first number must ==1)
  400f0e:	74 20 	je	32 <phase_2+0x34>       #if equal then go to 0x400F30
  400f10:	e8 25 05 00 00 	callq	1317 <explode_bomb>#if not,bommmm!
  400f15:	eb 19 	jmp	25 <phase_2+0x34>
  #************.   THE MOST IMPORTANT PART. *******************
  400f17:	8b 43 fc 	movl	-4(%rbx), %eax
  400f1a:	01 c0 	addl	%eax, %eax            # %eax*=2
  400f1c:	39 03 	cmpl	%eax, (%rbx)          # compare if input number is correct
  400f1e:	74 05 	je	5 <phase_2+0x29>        #if equal go to 400f25
  400f20:	e8 15 05 00 00 	callq	1301 <explode_bomb>
  400f25:	48 83 c3 04 	addq	$4, %rbx.       # get the next number(%rbx+=4)
  400f29:	48 39 eb 	cmpq	%rbp, %rbx.         #if the 6 numbers have been all compared
  400f2c:	75 e9 	jne	-23 <phase_2+0x1B>.     #go to 0x400F17
  400f2e:	eb 0c 	jmp	12 <phase_2+0x40>.      # go to 0x400f3c(return this function)
  #************************************************************
  400f30:	48 8d 5c 24 04 	leaq	4(%rsp), %rbx # pass the (%rsp+4) to %rbx
  400f35:	48 8d 6c 24 18 	leaq	24(%rsp), %rbp
  400f3a:	eb db 	jmp	-37 <phase_2+0x1B>.      #go to 0x400F17
...
phase_3:
  400f43:	48 83 ec 18 	subq	$24, %rsp           # %rsp stack: call this function
  400f47:	48 8d 4c 24 0c 	leaq	12(%rsp), %rcx.   # %rcx=(%rsp+12)
  400f4c:	48 8d 54 24 08 	leaq	8(%rsp), %rdx.    # %rdx=(%rsp+8)
  400f51:	be cf 25 40 00 	movl	$4203983, %esi.   # move addr=4203983 to %esi(%rsi)(int)
  400f56:	b8 00 00 00 00 	movl	$0, %eax          # %eax=0
  400f5b:	e8 90 fc ff ff 	callq	-880 <.plt+0x120> # internal function call
  400f60:	83 f8 01 	cmpl	$1, %eax                # if %eax > 1,then
  400f63:	7f 05 	jg	5 <phase_3+0x27>            # go to 0x400f6a
  400f65:	e8 d0 04 00 00 	callq	1232 <explode_bomb># %eax<=1,bommmmmm!!!
  400f6a:	83 7c 24 08 07 	cmpl	$7, 8(%rsp)       # 
  400f6f:	77 3c 	ja	60 <phase_3+0x6A>           # if (%rsp+8)>7,go to 0x400fad,bommmmm!!!!
  400f71:	8b 44 24 08 	movl	8(%rsp), %eax.      # %eax(%rax)=(%rsp+8)
  400f75:	ff 24 c5 70 24 40 00 	jmpq	*4203632(,%rax,8) #go to (8*%rax+0x402470)(switch)
  400f7c:	b8 cf 00 00 00 	movl	$207, %eax       # %eax=207
  400f81:	eb 3b 	jmp	59 <phase_3+0x7B>          # go to 0x400fbe
  400f83:	b8 c3 02 00 00 	movl	$707, %eax       # %eax=707
  400f88:	eb 34 	jmp	52 <phase_3+0x7B>          # go to 0x400fbe
  400f8a:	b8 00 01 00 00 	movl	$256, %eax
  400f8f:	eb 2d 	jmp	45 <phase_3+0x7B>          # go to 0x400fbe
  400f91:	b8 85 01 00 00 	movl	$389, %eax
  400f96:	eb 26 	jmp	38 <phase_3+0x7B>          # go to 0x400fbe
  400f98:	b8 ce 00 00 00 	movl	$206, %eax
  400f9d:	eb 1f 	jmp	31 <phase_3+0x7B>          # go to 0x400fbe
  400f9f:	b8 aa 02 00 00 	movl	$682, %eax
  400fa4:	eb 18 	jmp	24 <phase_3+0x7B>          # go to 0x400fbe
  400fa6:	b8 47 01 00 00 	movl	$327, %eax
  400fab:	eb 11 	jmp	17 <phase_3+0x7B>          # go to 0x400fbe
  400fad:	e8 88 04 00 00 	callq	1160 <explode_bomb>
  400fb2:	b8 00 00 00 00 	movl	$0, %eax         # %eax=0
  400fb7:	eb 05 	jmp	5 <phase_3+0x7B>           # go to 0x400fbe
  400fb9:	b8 37 01 00 00 	movl	$311, %eax
  400fbe:	3b 44 24 0c 	cmpl	12(%rsp), %eax     # (%rsp+12)==%eax???
  400fc2:	74 05 	je	5 <phase_3+0x86>           # true: go to 0x400fc9(return the function)
...
func4:
  ...
  400fd2:	89 d0 	movl	%edx, %eax            # %eax=%edx
  400fd4:	29 f0 	subl	%esi, %eax            # %eax-=%esi
  400fd6:	89 c1 	movl	%eax, %ecx            # %ecx=%eax
  400fd8:	c1 e9 1f 	shrl	$31, %ecx           # %ecx>>31(logical shift)
  400fdb:	01 c8 	addl	%ecx, %eax            # %eax+=%ecx
  400fdd:	d1 f8 	sarl	%eax                  # %eax/=2
  400fdf:	8d 0c 30 	leal	(%rax,%rsi), %ecx   # %ecx=(%rax+%rsi)
  400fe2:	39 f9 	cmpl	%edi, %ecx            # compare %ecx <= %edi
  400fe4:	7e 0c 	jle	12 <func4+0x24>         # true: go to 0x400ff2
  400fe6:	8d 51 ff 	leal	-1(%rcx), %edx      # %edx=%rcx-1
  400fe9:	e8 e0 ff ff ff 	callq	-32 <func4>   # call func4
  400fee:	01 c0 	addl	%eax, %eax            # %eax+=%eax
  400ff0:	eb 15 	jmp	21 <func4+0x39>         # go to 0x401007(return)
  400ff2:	b8 00 00 00 00 	movl	$0, %eax      # %eax=0
  400ff7:	39 f9 	cmpl	%edi, %ecx            # compare %ecx >= %edi
  400ff9:	7d 0c 	jge	12 <func4+0x39>         # true: return
  400ffb:	8d 71 01 	leal	1(%rcx), %esi       # %esi=%rcx+1
  400ffe:	e8 cb ff ff ff 	callq	-53 <func4>   # call func4
  401003:	8d 44 00 01 	leal	1(%rax,%rax), %eax # %eax=(%rax+%rax+1)
  401007:	48 83 c4 08 	addq	$8, %rsp        
  40100b:	c3 	retq

phase_4:
  40100c:	48 83 ec 18 	subq	$24, %rsp.        #    
  401010:	48 8d 4c 24 0c 	leaq	12(%rsp), %rcx  # %rcx=(%rsp+12)
  401015:	48 8d 54 24 08 	leaq	8(%rsp), %rdx.  # %rdx=(%rsp+8)
  40101a:	be cf 25 40 00 	movl	$4203983, %esi. # %esi(%rsi)=4203983
  40101f:	b8 00 00 00 00 	movl	$0, %eax        # %eax=0
  401024:	e8 c7 fb ff ff 	callq	-1081 <.plt+0x120> # call internal function(scanf)
  401029:	83 f8 02 	cmpl	$2, %eax              # %eax is the return value of scanf
  40102c:	75 07 	jne	7 <phase_4+0x29>          # if not equal=2, go to 0x401035(bomb!!!!!)
  40102e:	83 7c 24 08 0e 	cmpl	$14, 8(%rsp).   # 
  401033:	76 05 	jbe	5 <phase_4+0x2E>          # if (%rsp+8) <= 14, go to 0x40103a
  401035:	e8 00 04 00 00 	callq	1024 <explode_bomb> # else((%rsp+8)>14,then bomb!!!!!!)
  40103a:	ba 0e 00 00 00 	movl	$14, %edx.      # %edx=14
  40103f:	be 00 00 00 00 	movl	$0, %esi        # %esi=0
  401044:	8b 7c 24 08 	movl	8(%rsp), %edi     # %edi=(%rsp+8)
  401048:	e8 81 ff ff ff 	callq	-127 <func4>    # call func4
  40104d:	85 c0 	testl	%eax, %eax               
  40104f:	75 07 	jne	7 <phase_4+0x4C>          # if %eax!=0, go to 0x401058(bomb!!!!!!)
  401051:	83 7c 24 0c 00 	cmpl	$0, 12(%rsp)    # compare (%rsp+12)==0?
  401056:	74 05 	je	5 <phase_4+0x51>          # true:go to 0x40105d(return this function)
  401058:	e8 dd 03 00 00 	callq	989 <explode_bomb> # false: bommmmm!!!!!!
...
phase_5:
  ...
  401067:	48 89 fb 	movq	%rdi, %rbx             # %rbx=%rdi(your input is in %rbx now)
  40106a:	64 48 8b 04 25 28 00 00 00 	movq	%fs:40, %rax.  # %rax=%fs:40
  401073:	48 89 44 24 18 	movq	%rax, 24(%rsp)   # (%rsp+24)=%rax
  401078:	31 c0 	xorl	%eax, %eax               # %eax^=%eax(clear all the bits)
  40107a:	e8 9c 02 00 00 	callq	668 <string_length> # call string_length
  40107f:	83 f8 06 	cmpl	$6, %eax               # compare %eax==6
  401082:	74 4e 	je	78 <phase_5+0x70>          # true:go to 0x4010d2
  401084:	e8 b1 03 00 00 	callq	945 <explode_bomb>  # false:bommmmmmmmm!!!!
  401089:	eb 47 	jmp	71 <phase_5+0x70>          # go to 0x4010d2
  40108b:	0f b6 0c 03 	movzbl	(%rbx,%rax), %ecx # %ecx=(%rbx+%rax)(做了零拓展传到双字)
  40108f:	88 0c 24 	movb	%cl, (%rsp)            # (%rsp)=%cl(%rcx)(byte)
  401092:	48 8b 14 24 	movq	(%rsp), %rdx       # %rdx=(%rsp)
  401096:	83 e2 0f 	andl	$15, %edx              # %edx&=15(get the last 4 bits of your input number)
  401099:	0f b6 92 b0 24 40 00 	movzbl	4203696(%rdx), %edx # %edx=(%rdx+4203696)(in fact, we can view it as a array)
  4010a0:	88 54 04 10 	movb	%dl, 16(%rsp,%rax)  # (%rsp+%rax+16)=%dl(%rdx)(byte)
  4010a4:	48 83 c0 01 	addq	$1, %rax           # %rax+=1
  4010a8:	48 83 f8 06 	cmpq	$6, %rax           # compare if %rax!=6:
  4010ac:	75 dd 	jne	-35 <phase_5+0x29>         # true:go to 0x40108b
  4010ae:	c6 44 24 16 00 	movb	$0, 22(%rsp)     # (%rsp+22)=0(byte)
  4010b3:	be 5e 24 40 00 	movl	$4203614, %esi   # %esi=4203614
  4010b8:	48 8d 7c 24 10 	leaq	16(%rsp), %rdi   # %rdi=(%rsp+16)
  4010bd:	e8 76 02 00 00 	callq	630 <strings_not_equal> # call strings_not_equal
  4010c2:	85 c0 	testl	%eax, %eax               # if return value==0
  4010c4:	74 13 	je	19 <phase_5+0x77>          # true: go to 0x4010d9
  4010c6:	e8 6f 03 00 00 	callq	879 <explode_bomb> # false: bommmmmm!!!!
  4010cb:	0f 1f 44 00 00 	nopl	(%rax,%rax)      # no effective operation
  4010d0:	eb 07 	jmp	7 <phase_5+0x77>           # go to 0x4010d9
  4010d2:	b8 00 00 00 00 	movl	$0, %eax         # %eax=0
  4010d7:	eb b2 	jmp	-78 <phase_5+0x29>         # go to 0x40108b
  4010d9:	48 8b 44 24 18 	movq	24(%rsp), %rax   # %rax=(%rsp+24)
  4010de:	64 48 33 04 25 28 00 00 00 	xorq	%fs:40, %rax  # rax^=%fs:40==0?
  4010e7:	74 05 	je	5 <phase_5+0x8C>           # true: go to 0x4010ee(return)
...
phase_6:
  ...
  401100:	49 89 e5 	movq	%rsp, %r13                    # %r13=%rsp(now %rsp and %r13 store the input numbers)
  401103:	48 89 e6 	movq	%rsp, %rsi                    # %rsi=%rsp
  401106:	e8 51 03 00 00 	callq	849 <read_six_numbers>  # call read_six_numbers function
  40110b:	49 89 e6 	movq	%rsp, %r14                    # %r14=%rsp
  40110e:	41 bc 00 00 00 00 	movl	$0, %r12d           # %r12d=0
  # -------------THIS IS A BIGGER LOOP------------------
  401114:	4c 89 ed 	movq	%r13, %rbp                    # %rbp=%r13
  401117:	41 8b 45 00 	movl	(%r13), %eax              # %eax=(%r13)
  40111b:	83 e8 01 	subl	$1, %eax                      # %eax-=1
  40111e:	83 f8 05 	cmpl	$5, %eax                      # if %eax<=5
  401121:	76 05 	jbe	5 <phase_6+0x34>                  # true: go to 0x401128
  401123:	e8 12 03 00 00 	callq	786 <explode_bomb>      # false: bommmm!!!!!!
  401128:	41 83 c4 01 	addl	$1, %r12d                 # %r12d+=1
  40112c:	41 83 fc 06 	cmpl	$6, %r12d                 # if %r12d==6?
  401130:	74 21 	je	33 <phase_6+0x5F>                 # true: go to 0x401153
  401132:	44 89 e3 	movl	%r12d, %ebx                   # %ebx=%r12d
  401135:	48 63 c3 	movslq	%ebx, %rax                  # %rax=%ebx(做补零)
  #************ THIS IS A SMALL LOOP********************
  401138:	8b 04 84 	movl	(%rsp,%rax,4), %eax           # %eax=%rsp[%rax](in fact, the original
                                                        # clause is (%rsp+4*%rax))
  40113b:	39 45 00 	cmpl	%eax, (%rbp)                  # if (%rbp)!=%eax(which means the input
                                                        # can not be the same number)
  40113e:	75 05 	jne	5 <phase_6+0x51>                  # true: go to 0x401145
  401140:	e8 f5 02 00 00 	callq	757 <explode_bomb>      # false: bommmmmm!!!!!
  401145:	83 c3 01 	addl	$1, %ebx                      # %ebx+=1
  401148:	83 fb 05 	cmpl	$5, %ebx                      # if %ebx <= 5 ?
  40114b:	7e e8 	jle	-24 <phase_6+0x41>                # true: go to 0x401135
  #*****************************************************
  40114d:	49 83 c5 04 	addq	$4, %r13                  # %r13+=4(find the next number)
  401151:	eb c1 	jmp	-63 <phase_6+0x20>                # go to 0x401114
  #-----------------------------------------------------
  # The above part is to check if the input number have the same number
  401153:	48 8d 74 24 18 	leaq	24(%rsp), %rsi          # %rsi=%rsp+24(get the last input number)
  401158:	4c 89 f0 	movq	%r14, %rax                    # %rax=%r14(the first input number address)
  40115b:	b9 07 00 00 00 	movl	$7, %ecx                # %ecx=7       
  # ######## This is a small loop ,#####################
  # the purpose of this small loop is to let every input number x=7-x
  401160:	89 ca 	movl	%ecx, %edx                      # %edx=%ecx(%edx=7)
  401162:	2b 10 	subl	(%rax), %edx                    # %edx-=(%rax)
  401164:	89 10 	movl	%edx, (%rax)                    # (%rax)=%edx
  401166:	48 83 c0 04 	addq	$4, %rax                  # %rax+=4(points to the second number)
  40116a:	48 39 f0 	cmpq	%rsi, %rax                    # if %rax!=%rsi(if we havent reach the last number)
  40116d:	75 f1 	jne	-15 <phase_6+0x6C>                # true: go to 0x401160
  # ####################################################
  40116f:	be 00 00 00 00 	movl	$0, %esi                # %esi=0
  401174:	eb 21 	jmp	33 <phase_6+0xA3>                 # go to 0x401197
  # @@@@@@@@@@@@@@@@@ small loop @@@@@@@@@@@@@@@@@@@@@@@
  401176:	48 8b 52 08 	movq	8(%rdx), %rdx             # %rdx=(%rdx+8)
  40117a:	83 c0 01 	addl	$1, %eax                      # %eax+=1
  40117d:	39 c8 	cmpl	%ecx, %eax                      # %eax != %ecx
  40117f:	75 f5 	jne	-11 <phase_6+0x82>                # true: go to 0x401176
  # @ This loop is to get the node at which the input number is @
  401181:	eb 05 	jmp	5 <phase_6+0x94>                  # go to 0x401188
  401183:	ba d0 32 60 00 	movl	$6304464, %edx          # %edx=6304464
  401188:	48 89 54 74 20 	movq	%rdx, 32(%rsp,%rsi,2)   # (%rsp+2*%rsi+32)=%rdx
  40118d:	48 83 c6 04 	addq	$4, %rsi                  # %rsi+=4
  401191:	48 83 fe 18 	cmpq	$24, %rsi                 # if %rsi==24
  401195:	74 14 	je	20 <phase_6+0xB7>                 # true: go to 0x4011ab
  401197:	8b 0c 34 	movl	(%rsp,%rsi), %ecx             # %ecx=(%rsp+%rsi)
  40119a:	83 f9 01 	cmpl	$1, %ecx                      # if %ecx <= 1
  40119d:	7e e4 	jle	-28 <phase_6+0x8F>                # true: go to 0x401183
  40119f:	b8 01 00 00 00 	movl	$1, %eax                # %eax=1
  4011a4:	ba d0 32 60 00 	movl	$6304464, %edx          # %edx=6304464
  4011a9:	eb cb 	jmp	-53 <phase_6+0x82>                # go to 0x401176

  4011ab:	48 8b 5c 24 20 	movq	32(%rsp), %rbx          # %rbx= (%rsp+32) (first node address)
  4011b0:	48 8d 44 24 28 	leaq	40(%rsp), %rax          # %rax= %rsp+40 (points to the second one)
  4011b5:	48 8d 74 24 50 	leaq	80(%rsp), %rsi          # %rsi= %rsp+80 (get the end addr)
  4011ba:	48 89 d9 	movq	%rbx, %rcx                    # %rcx=%rbx(now %rcx points to the first node)
  4011bd:	48 8b 10 	movq	(%rax), %rdx                  # %rdx=(%rax)(%rdx is the second node)
  4011c0:	48 89 51 08 	movq	%rdx, 8(%rcx)             # (%rcx+8)=%rdx(modify the next pointer)
  4011c4:	48 83 c0 08 	addq	$8, %rax                  # %rax+=8 (%rax points to the third number)
  4011c8:	48 39 f0 	cmpq	%rsi, %rax                    # if %rax == %rsi(if have traverse the whole numbers)
  4011cb:	74 05 	je	5 <phase_6+0xDE>                  # true: go to 0x4011d2
  4011cd:	48 89 d1 	movq	%rdx, %rcx                    # %rcx=%rdx
  4011d0:	eb eb 	jmp	-21 <phase_6+0xC9>                # go to 0x4911bd
  4011d2:	48 c7 42 08 00 00 00 00 	movq	$0, 8(%rdx)   # (%rdx+8)=0
  4011da:	bd 05 00 00 00 	movl	$5, %ebp                # %ebp=5
  4011df:	48 8b 43 08 	movq	8(%rbx), %rax             # %rax=(%rbx+8)
  4011e3:	8b 00 	movl	(%rax), %eax                    # %eax=(%rax)
  4011e5:	39 03 	cmpl	%eax, (%rbx)                    # if (%rbx) >= %eax
  4011e7:	7d 05 	jge	5 <phase_6+0xFA>                  # true:go to 0x4011ee
  4011e9:	e8 4c 02 00 00 	callq	588 <explode_bomb>      # false:bommmm!!!!!!
  4011ee:	48 8b 5b 08 	movq	8(%rbx), %rbx             # %rbx=(%rbx+8)
  4011f2:	83 ed 01 	subl	$1, %ebp                      # %ebp-=1
  4011f5:	75 e8 	jne	-24 <phase_6+0xEB>                # if %eax!=(%rbx) go to 0x4011df       
  4011f7:	48 83 c4 50 	addq	$80, %rsp                 # %rsp+=80 return
  4011fb:	5b 	popq	%rbx
  4011fc:	5d 	popq	%rbp
...
secret_phase:
  401242:	53 	pushq	%rbx
  401243:	e8 56 02 00 00 	callq	598 <read_line>   # call function read_line
  401248:	ba 0a 00 00 00 	movl	$10, %edx         # %edx=10
  40124d:	be 00 00 00 00 	movl	$0, %esi          # %esi=0
  401252:	48 89 c7 	movq	%rax, %rdi              # %rdi=%rax
  401255:	e8 76 f9 ff ff 	callq	-1674 <.plt+0x100>  # scanf
  40125a:	48 89 c3 	movq	%rax, %rbx              # %rbx=%rax
  40125d:	8d 40 ff 	leal	-1(%rax), %eax          # %rax=%rax-1
  401260:	3d e8 03 00 00 	cmpl	$1000, %eax       # %rax >= 1000?
  401265:	76 05 	jbe	5 <secret_phase+0x2A>       # true: go to 0x40126c
  401267:	e8 ce 01 00 00 	callq	462 <explode_bomb># false: bommmmm!!!
...
string_length:
  40131b:	80 3f 00 	cmpb	$0, (%rdi)     # (%rdi) stores the string
...
strings_not_equal:
  ...
  40133a:	55 	pushq	%rbp
  40133b:	53 	pushq	%rbx
  40133c:	48 89 fb 	movq	%rdi, %rbx    # %rbx=%rdi
  40133f:	48 89 f5 	movq	%rsi, %rbp    # %rbp=%rsi
  401342:	e8 d4 ff ff ff 	callq	-44 <string_length>
  401347:	41 89 c4 	movl	%eax, %r12d   # %r12d=the length of string
  40134a:	48 89 ef 	movq	%rbp, %rdi    # %rdi=%rbp
...
read_six_numbers:
  ...
  40148a:	e8 61 f7 ff ff 	callq	-2207 <.plt+0x120>    # call jmpl *2106514(%rip)？？？迷
  40148f:	83 f8 05 	cmpl	$5, %eax                    #
  401492:	7f 05 	jg	5 <read_six_numbers+0x3D>       #here <> is to explain number 5(if %eax>5,go to 401499)
                                                      #which means the number of input number must be 6
  401494:	e8 a1 ff ff ff 	callq	-95 <explode_bomb>    # if not, bomb!!!
